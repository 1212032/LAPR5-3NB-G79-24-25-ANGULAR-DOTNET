...
% marcacoes do staff
agenda_staff(d001,20241028,[(560,630,m01)]).
...
evaluate_population([Ind|Rest],[Ind*V|Rest1]) :-
    ( schedule_all_surgeries(Ind, 20241028, V) -> true
...
generate_generation(N, MaxGen, Pop, BestSoFar, StagnationCount):-
    ...
    ;
    (N1 is N + 1,
    generate_generation(N1, MaxGen, NewPopOrd, NewBest, UpdatedCount))).
...
schedule_all_surgeries(LOpCode,Day,Max):-
    ...
    availability_all_surgeries(LOpCode,Day),
	
	write("Individual: "),write(LOpCode),
	findall(_,(operation_room(Room),agenda_operation_room1(Room,20241028,Agenda),write("  Room "),write(Room),write(" agenda: "),write(Agenda)),_),nl,
	find_max_end_time(Day, Max).


find_max_end_time(Day, MaxEndTime) :-
	findall(Room,operation_room(Room),Rooms),
    findall(EndTime, 
        (member(Room, Rooms), agenda_operation_room1(Room, Day, Agenda), get_end_value(Agenda, _, EndTime)), 
        EndTimes),
    max_member(MaxEndTime, EndTimes).
...
find_best_room(RoomSlots, SelectedRoom) :-
    % Calcula a soma total de tempos livres para cada sala
    maplist(add_total_free_time, RoomSlots, RoomTimeCounts),
    % Ordena pela soma total de tempos livres (decrescente)
    keysort(RoomTimeCounts, SortedRoomTimes),
    reverse(SortedRoomTimes, DescSortedRoomTimes),
    % Seleciona a sala com mais tempo livre
    member(_-SelectedRoom, DescSortedRoomTimes).

% Calcula a soma total dos tempos livres e cria pares TotalTime-Room
add_total_free_time(Room-FreeSlots, TotalTime-Room) :-
    sum_free_slots(FreeSlots, TotalTime).

% Soma o tempo de todos os intervalos livres
sum_free_slots([], 0).
sum_free_slots([(Start, End)|Rest], TotalTime) :-
    Interval is End - Start + 1, % Calcula o tempo livre no intervalo
    sum_free_slots(Rest, RestTime),
    TotalTime is Interval + RestTime.



availability_all_surgeries([],_).
availability_all_surgeries([OpCode|LOpCode],Day):-
	select_operation_room(Day,Room),
	agenda_operation_room1(Room, Day, Agenda),
